...
import {addTask, delTask, getBusinessList, getTask, getTaskDetails, listTask, updateTask,} from '@/api/manage/task';
...
function handleDelete(row) {
  const _taskIds = row?.taskId ? [row.taskId.toString()] : ids.value.map(String);

  const invalidTasks = taskList.value.filter(task =>
      _taskIds.includes(task.taskId.toString()) &&
      ![3, 4].includes(task.taskStatus)
  );

  if (invalidTasks.length > 0) {
    proxy.$modal.msgError('只有状态为【取消】或【完成】的工单可删除');
    return;
  }

  proxy.$modal
      .confirm('是否确认删除选中的' + _taskIds.length + '条工单数据？')
      .then(function () {
        return delTask(_taskIds.join(','));
      })
      .then(() => {
        getList();
      ...
      .catch((e) => {
        if(e.message.includes('存在关联数据')) {
          proxy.$modal.msgError('删除失败：请先解除工单相关数据关联');
        }
      });
}
